...
def _robustness_map(path: str, rows: list[dict[str, Any]], metric: str, title: str) -> None:
    ...
    for row in selected:
        ...
        popup = (
            ...
            f"eff_retained={float(row['efficiency_topological_retained']):.4f}"
        )
        folium.Rectangle(
...
def simular_robustez_espacial(
    ...
    rows.sort(
        key=lambda row: (
            ...
            1.0 - row["efficiency_topological_retained"],
            row["removed_edges"],
        ),
        reverse=True,
    )
    ...
    if evaluate_efficiency:
        ...
        H = G.copy()
        for row in rows[:max_efficiency_cells]:
    ...
    for row in rows:
        row["efficiency_topological_drop"] = 1.0 - float(row["efficiency_topological_retained"])
